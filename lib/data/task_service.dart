...
class TasksService {
  ...
  TasksService({required this.dio}) {
    ...
    dio.interceptors.add(InterceptorsWrapper(onRequest: (options, handler) {
      options.headers['Authorization'] = 'Bearer Tulkas';
  ...
  Future<Task> deleteTask(String taskId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<Task> currentTasks = await getAllTasks();
        currentTasks.removeWhere((task) => task.UUID == taskId);
  ...
  Future<Task> editTask(Task task) async {
    try {
      ...
      final taskID = task.UUID.replaceAll(RegExp(r'[\[\]#]'), '');
      ...
      if (response.statusCode == 200) {
        ...
        final int index = currentTasks.indexWhere((t) => t.UUID == task.UUID);
        if (index != -1) {
